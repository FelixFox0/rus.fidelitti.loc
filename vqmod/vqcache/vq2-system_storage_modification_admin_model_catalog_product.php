...
// FOR CUSTOM SORT
		if (isset($data['sort_order_product_cat_id'])) {
			foreach ($data['sort_order_product_cat_id'] as $cat_id => $value) { 
				$this->db->query("UPDATE " . DB_PREFIX . "product_to_category SET sort_order = '" . (int)$value . "' WHERE product_id = '" . (int)$product_id . "' AND category_id = '" . (int)$cat_id . "'");
			}
		}
// FOR CUSTOM SORT
...
// FOR CUSTOM SORT
		if (isset($data['sort_order_product_cat_id'])) {
			foreach ($data['sort_order_product_cat_id'] as $cat_id => $value) { 
				$this->db->query("UPDATE " . DB_PREFIX . "product_to_category SET sort_order = '" . (int)$value . "' WHERE product_id = '" . (int)$product_id . "' AND category_id = '" . (int)$cat_id . "'");
//			$this->db->query("UPDATE " . DB_PREFIX . "product_to_category SET sort_order = '9' WHERE product_id = '47' AND category_id = '20'");
			}
		}
// FOR CUSTOM SORT
...
//BOF Product Series	
	...
	public function getProducts($data = array()) {
		...
		$sort_data = array(
			...
			'p.status',
				...
				'p2c.sort_order',//FOR DRAG&DROP SORT
	...
	public function updateTable() {
		$query = $this->db->query("SELECT count(*) as c FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='" . DB_DATABASE . "' AND TABLE_NAME='" . DB_PREFIX . "product_to_category' AND COLUMN_NAME='sort_order'");

		if (!$query->row['c']){
			$query = $this->db->query("ALTER TABLE " . DB_PREFIX . "product_to_category ADD sort_order int(11) NOT NULL DEFAULT 10000"); 
		}
		return $query;
	}
...
// FOR CUSTOM SORT
	public function getProductSortOrder($product_id) {

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' ORDER BY product_id");
		
		return $query->rows;
	}

	public function setProductSortOrder($product_id,$category_id,$sort_order) {

		$query = $this->db->query("UPDATE " . DB_PREFIX . "product_to_category SET sort_order = '" . (int)$sort_order . "' WHERE product_id = '" . (int)$product_id . "' AND category_id = '" . (int)$category_id . "'");
		
		return $query;
	}

	public function getProductSortOrd($product_id,$category_id) {

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id = '" . (int)$category_id . "' ORDER BY product_id");
